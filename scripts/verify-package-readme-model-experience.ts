...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-color-accents': { kind: 'none', reason: 'Browser-side theme accents and a token layer; registers nothing model-facing.' },
  'packages/client/ui-power': { kind: 'none', reason: 'Browser-side power button that only issues host process-lifecycle calls; registers nothing model-facing.' },
  ...
  'packages/host/power': { kind: 'none', reason: 'The Remote only performs host process-lifecycle actions; registers nothing model-facing.' },
  ...
  'packages/bundle/power': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its model-facing behavior.' },
